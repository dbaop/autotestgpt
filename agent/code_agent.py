    ...
    def __init__(self):
        ...
        if self.custom_system_prompt:
            ...
            self.ui_test_prompt = (
                ...
                "Return strict JSON with key `scripts` as a list of script objects.\n\n"
                "**BUSINESS FEATURE FOCUS**: Each script must test the SPECIFIC business feature "
                "described in the test case (e.g., '推送配置', '日报预览', '历史记录', '站内消息'), "
                "NOT generic page loads or login flows. The `given.url` should navigate to the "
                "actual feature page (e.g., '/daily-report/push-config'), not the login page. "
                "If login is required, it should be handled in preconditions, not as part of the test flow.\n\n"
